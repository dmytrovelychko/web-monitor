...
class SimpleApiRouter {
	...
	async [generalHandler](req, res, buffer) {
		...
		log.info(`Returning this response: ${result.statusCode}, ${payloadString}`);
	}
	...
	async [callHandlersChain](method, path, ctx) {
		...
		const { statusCode, payload } = await fn();
...
function parseUrl(reqUrl) {
	...
	for (const prop of Object.getOwnPropertyNames(parsedUrl.query)) {
		try {
			queryStringParams[prop] = parseInt(queryStringParams[prop], 10);
		} catch (err) {
			// ignore
		}
	}
