...
class SimpleApiRouter {
	...
	/* @TODO we can decline request processing right away if method / handler
	        not found before fill in all buffer */
	router(req, res) {
	...
	async [generalHandler](req, res, buffer) {
		...
		try {
			...
			// @TODO provide list of allowed method for provided uri
		...
		const payloadString = JSON.stringify(
			typeof result.payload === 'object' ? result.payload : {},
		);
...
// @TODO create proper new error types
function notAllowed() {
